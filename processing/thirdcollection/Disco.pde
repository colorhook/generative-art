...
void createTree() {
   ...
   _strutFactor = (noise(_noiseoff) * 0.5) - 0.1;
   
   rotateStartPoints();
   
   _branchArr = (Branch[])expand(_branchArr, 0);
   Branch root = new Branch(0, 1, _pointArr);
   // println("Branches created:  " + _branchArr.length);
...
class Branch {
  ...
  void drawme() {
    // Draw the branch
     stroke(random(255), random(255), random(255), level * 20);
   strokeWeight((_maxlevels - level) * 3);
