...
void setup() { 
  ...
  img = createImage(_numX, _numY, RGB);
...
void draw() {
  ...
  // draw all the cells in the array into an image
  int i = 0;
    for (int y = 0; y < _numY; y++) {
  for (int x = 0; x < _numX; x++) {
       img.pixels[i] = _cellArray[x][y].getMyCol();
       i++;
    }
  }
  img.updatePixels();
  image(img, 0, 0, _numX, _numY);
}
...
int xmin, xmax, ymin, ymax;
  ...
  void drawMe() {
    state = nextState;
    color col = color(colArr[_ycol][int(state)]);
    fill(col, 100);   // use the state value as the fill colour 
    stroke(col); 
    point(x,y);
  }
